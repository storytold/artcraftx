...
mod tests {
  ...
  use fal_client::requests::api::video::text::vidu_q3::api::{
    ViduQ3TextToVideoAspectRatio, ViduQ3TextToVideoResolution,
  };
  ...
  async fn send_text_to_video_webhook() {
    let state = FalViduQ3RequestState {
      mode: FalViduQ3Mode::TextToVideo(ViduQ3TextToVideoRequest {
  ...
  async fn send_image_to_video_webhook() {
    let state = FalViduQ3RequestState {
      mode: FalViduQ3Mode::ImageToVideo(ViduQ3ImageToVideoRequest {
