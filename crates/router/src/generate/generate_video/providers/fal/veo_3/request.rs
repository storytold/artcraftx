...
mod tests {
  ...
  use fal_client::requests::api::video::image::veo_3::api::{
    Veo3ImageToVideoDuration, Veo3ImageToVideoResolution,
  };
  use fal_client::requests::api::video::text::veo_3::api::{
    Veo3TextToVideoDuration, Veo3TextToVideoResolution,
  };
  ...
  async fn live_text_to_video_720p_4s_audio_on() {
    let state = FalVeo3RequestState {
      mode: FalVeo3Mode::TextToVideo(Veo3TextToVideoRequest {
  ...
  async fn live_image_to_video_1080p_8s_no_audio() {
    let state = FalVeo3RequestState {
      mode: FalVeo3Mode::ImageToVideo(Veo3ImageToVideoRequest {
        prompt: "the dog leaps into the lake.".to_string(),
        image_url: JUNO_AT_LAKE_IMAGE_URL.to_string(),
        aspect_ratio: None,
        duration: Some(Veo3ImageToVideoDuration::EightSeconds),
