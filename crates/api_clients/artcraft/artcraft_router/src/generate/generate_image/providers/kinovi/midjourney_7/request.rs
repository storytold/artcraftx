use seedance2pro_client::generate::image::generate_midjourney_v7::{
  generate_midjourney_v7, GenerateMidjourneyV7Args, GenerateMidjourneyV7Request,
};
...
impl KinoviMidjourney7RequestState {
  pub async fn send(
    ...
    let args = GenerateMidjourneyV7Args {
      session: &client.session,
      host_override: None,
      request: self.request.clone(),
    };

    let response = generate_midjourney_v7(args)
      .await
...
mod tests {
  ...
  fn base_builder() -> GenerateImageRequestBuilder {
    GenerateImageRequestBuilder {
      ...
      prompt: Some("a serene mountain lake at sunrise, cinematic".to_string()),
      image_inputs: None,
      resolution: None,
  ...
  async fn image_reference_round_trip_via_draft() {
    ...
    let builder = GenerateImageRequestBuilder {
      prompt: Some("a corgi in the style of @1".to_string()),
