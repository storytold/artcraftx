...
pub enum GrokSpecificApiError {
  ...
  /// The requested video resolution (e.g. `1080p`) is not available on the
  /// account's current plan. Observed as a 403 whose body reads
  /// "1080p video generation is not available on your current plan. Use 720p
  /// instead." Carries the HTTP status and full response body.
  VideoResolutionNotAllowed {
    status_code: u16,
    body: String,
  },
...
impl Display for GrokSpecificApiError {
  fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
    match self {
      ...
      Self::VideoResolutionNotAllowed { status_code, body } => write!(f, "Video resolution not available on the current plan (HTTP {}). Body: {}", status_code, body),
