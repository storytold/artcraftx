...
pub fn build_artcraft_suno_sample(builder: GenerateAudioRequestBuilder) -> Result<AudioGenerationDraftOrRequest, ArtcraftRouterError> {
  let request = build_artcraft_omni_audio_request(builder, CommonAudioModelEnum::SunoSample)?;
  let state = ArtcraftSunoSampleRequestState { request };
  Ok(AudioGenerationDraftOrRequest::Request(AudioGenerationRequest::ArtcraftSunoSample(state)))
}
